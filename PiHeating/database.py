...
class DbUtils():
    ...
    def getTemps(self, roomName, currentTime):
        ...
        cursor = conn.cursor()
        with conn:
            cursor.execute("""SELECT * FROM temps WHERE RoomName = '{}' AND time > {}""".format(
                roomName.replace("'", "''"), currentTime))
            variables = cursor.fetchall()
            return variables
    ...
    def getLastTemp(self, roomName):
        ...
        with conn:
            cursor.execute(
                """SELECT ActTemp FROM temps WHERE RoomName = '{}' AND ActTemp > 0.0 order by time desc limit 1""".format(roomName))
            variables = cursor.fetchall()
    ...
    def getBoiler(self):
        ...
        with conn:
            cursor.execute(
                "SELECT * FROM boiler WHERE ID = (SELECT MAX(ID) FROM boiler)")
            variables = cursor.fetchall()
            return variables[0]
    ...
    def getTimedBoiler(self, timeLimit):
        ...
        with conn:
            cursor.execute(
                "SELECT * FROM boiler WHERE Time > {}".format(timeLimit))
            variables = cursor.fetchall()
            return variables
